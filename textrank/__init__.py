...
def mecab_tokenizer(sent):
    ...
    words = [w for w in words if ('/NNG' in w or '/NNP' in w or '/NP' in w)]
    return words
...
def extract_keyword(content):
    ...
    # 키워드 뽑기
    keywords = keyword_extractor.summarize(content, topk=10)
    for word, rank in keywords:
        print(word, rank)

def extract_keysentence(content):
    # 키문장 추출 모델
    keysentence_extractor = KeysentenceSummarizer(
        tokenize = mecab_tokenizer,
        min_sim = 0.5,
        verbose = True
    )
    ...
    # 문장 뽑기
    keysents = keysentence_extractor.summarize(content, topk=5)
...
if __name__ == '__main__':
    ...
    # 키워드 추출
    extract_keyword(texts)

    # 키문장 추출
    extract_keysentence(texts)